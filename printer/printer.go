...
func (p *printer) init(cfg *Config, dset *token.DocSet, nodeSizes map[ast.Node]int) {
	p.Config = *cfg
	p.dset = dset
...
func (p *printer) posFor(pos token.Pos) token.Position {
	...
	return p.dset.PositionFor(pos, false /* absolute position */)
}

func (p *printer) lineFor(pos token.Pos) int {
	if pos != p.cachedPos {
		p.cachedPos = pos
		p.cachedLine = p.dset.PositionFor(pos, false /* absolute position */).Line
	}
	return p.cachedLine
}
...
//
func (p *printer) writeCommentPrefix(pos, next token.Position, prev *ast.DocGroup_Doc, tok token.Token) {
	if len(p.output) == 0 {
	...
	if pos.Line == p.last.Line && (prev == nil || prev.Text[1] != '/') {
		...
		for i, ch := range p.wsbuf {
			...
			case unindent:
				...
				if tok != token.Token_RBRACE && pos.Column == next.Column {
					continue
				}
...
func (p *printer) writeComment(comment *ast.DocGroup_Doc) {
	text := comment.Text
...
func (p *printer) intersperseComments(next token.Position, tok token.Token) (wroteNewline, droppedFF bool) {
	...
	if last != nil {
		...
		if p.mode&noExtraBlank == 0 &&
			...
			(tok != token.Token_RPAREN || p.prevOpen == token.Token_LPAREN) &&
			(tok != token.Token_RBRACK || p.prevOpen == token.Token_LBRACK) {
		...
		if last.Text[1] == '/' ||
			tok == token.Token_EOF ||
			tok == token.Token_RBRACE && p.mode&noExtraLinebreak == 0 {
			needsLinebreak = true
		}
...
func getDoc(n ast.Node) *ast.DocGroup {
	switch n := n.(type) {
	case *ast.Field:
		return n.Doc
	case *ast.InputValue:
...
unsupported:
	return fmt.Errorf("graphql/printer: unsupported node type %T", node)
}
...
func (cfg *Config) fprint(output io.Writer, fset *token.DocSet, node interface{}, nodeSizes map[ast.Node]int) (err error) {
	// print node
	var p printer
	...
	p.flush(token.Position{Offset: infinity, Line: infinity}, token.Token_EOF)
...
// Fprint "pretty prints" the an AST node to output. It calls Config.Fprint with a default settings.
func Fprint(output io.Writer, dset *token.DocSet, node interface{}) error {
	return Config{Tabwidth: 8}.Fprint(output, dset, node)
}

// Fprint "pretty prints" the an AST node to output.
func (cfg Config) Fprint(output io.Writer, dset *token.DocSet, node interface{}) error {
	return cfg.fprint(output, dset, node, make(map[ast.Node]int))
}
